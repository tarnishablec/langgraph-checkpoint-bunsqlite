...
/**
 * Internal row structure for the checkpoints table
 */
...
export class BunSqliteSaver extends BaseCheckpointSaver {
     ...
     *
     * @param connString - Path to the SQLite database file (use ``:memory:`` for an in-memory database)
    ...
    private setupTables(): void {
        ...
        // Create a checkpoint_writes table for pending writes
        this.db.run(`
        ...
        // Create an index for efficient write queries
        this.db.run(`
    ...
    /**
     * Deserialize a checkpoint row from the database into a CheckpointTuple
     *
     * @param row - The database row to deserialize
     * @returns The deserialized checkpoint tuple
     */
    private async loadCheckpoint(row: CheckpointRow): Promise<CheckpointTuple> {
    ...
    async getTuple(config: RunnableConfig): Promise<CheckpointTuple | undefined> {
        ...
        return this.loadCheckpoint(row);
    }
    ...
    async* list(
        ...
        for (const row of rows) {
            const tuple = await this.loadCheckpoint(row);

            // Check filter if specified
            if (options?.filter !== undefined) {
                const metadataRecord = tuple.metadata as unknown as Record<string, unknown>;
    ...
    async putWrites(
        ...
        const stmt = this.db.prepare(`
            INSERT
            OR REPLACE INTO checkpoint_writes
